...
bool check_quantized_mixed_linear_args(
  ...
  ET_LOG_AND_RETURN_IF_FALSE(out.scalar_type() == out_dtype)
  ...
  ET_LOG_MSG_AND_RETURN_IF_FALSE(
      ...
      "out_dtype must be Float or Half");
...
Tensor& quantized_mixed_linear_out(
  ...
  ET_KERNEL_CHECK(
      ...
      check_quantized_mixed_mm_args(
          in, weight, weight_scales, opt_weight_zero_points, out_dtype, out),
      InvalidArgument,
      out);
  ...
  ET_SWITCH_TWO_TYPES(Float, Half, in.scalar_type(), ctx, name, CTYPE, [&]() {
    ...
    // FIXME: this currently ignores out_dtype
    vec_quantized_matmul_transb_int8<CTYPE, int8_t, CTYPE>(
...
Tensor& quantized_mixed_linear_out(
  ...
  // TODO(mkg): add support for out_dtype
  (void)ctx;
